...
class TestLeadDepartureController(OpenpilotTestCase):
  ...
  def test_slow_lead_creep_never_arms(self):
    controller = LeadDepartureController(True)
    for frame in range(20):
      lead = make_lead(d_rel=4.0 + frame * 0.03, v_lead=LEAD_DEPARTURE_MIN_SPEED - 0.05, v_rel=LEAD_DEPARTURE_MIN_SPEED - 0.05)
      assert update(controller, make_sm(lead_one=lead))
    assert not controller.active
  ...
  def test_closed_loop_departure_releases_earlier_without_a_safety_regression(self):
    ...
    assert controller_release <= stock_release - 0.5
    assert stock_motion - controller_motion >= DT_MDL
    assert stock_v01 - controller_v01 >= DT_MDL
